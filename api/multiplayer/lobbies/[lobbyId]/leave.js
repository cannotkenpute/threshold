/**
 * POST /api/multiplayer/lobbies/:lobbyId/leave — idempotent (leave_multiplayer_lobby
 * no-ops if the caller isn't an active member). Also handles host migration /
 * lobby closure server-side when the leaver was host or the last member.
 */
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data, error } = await client.rpc('leave_multiplayer_lobby', { p_lobby_id: lobbyId });
    if (error) {
